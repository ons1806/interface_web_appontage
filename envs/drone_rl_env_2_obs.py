...
class DroneLandingRLEnv(gym.Env):
    ...
    def __init__(
        ...
        # Espace des actions
        # Action = [dx_cmd, dy_cmd, dz_cmd]
        # dx_cmd, dy_cmd = petites corrections horizontales autour du point d’appontage
        # dz_cmd = petite correction verticale autour du point d’appontage
        self.command_low = np.array([-0.08, -0.08, -0.06], dtype=np.float32)
        self.command_high = np.array([0.08, 0.08, 0.015], dtype=np.float32)

        if self.normalized_action_input:
            # PPO travaille dans [-1, 1], puis l'environnement convertit en metres.
    ...
    def _init_platform_motion(self):
        ...
        if self.platform_motion_mode == 0:
            ...
            self.motion_Rx = (angle_max_Rx / 6.0) * np.sin(
    ...
    def step(self, action: np.ndarray):
        ...
        """
        # Convertit l'action PPO en correction metrique envoyee au PID.
        ...
        # L'action du PPO représente une petite correction autour du point d’appontage
        # [dx_cmd, dy_cmd, dz_cmd]
        dx_cmd, dy_cmd, dz_cmd = action
        ...
        # la cible suit la plateforme, avec une petite correction choisie par l'agent.
        # On limite la cible verticale pour eviter une consigne de descente trop
        # brutale au debut de l'episode.
        raw_target_z = max(
            float(self.landing_target[2] + dz_cmd),
        ...
        limited_target_z = max(raw_target_z, current_z - descent_rate * dt)
        ...
        target_pos = np.array([
            self.landing_target[0] + dx_cmd,
            self.landing_target[1] + dy_cmd,
            limited_target_z,
        ], dtype=np.float32)
        ...
        sat_penalty = (
           float(abs(dx_cmd) > 0.055)
           + float(abs(dy_cmd) > 0.055)
           + float(dz_cmd > 0.004)
           + float(dz_cmd < -0.030)
        )
        ...
        info = {
            ...
            "policy_action_dx": float(self.last_policy_action[0]),
            "policy_action_dy": float(self.last_policy_action[1]),
            "policy_action_dz": float(self.last_policy_action[2]),
            "command_dx": float(dx_cmd),
            "command_dy": float(dy_cmd),
            "command_dz": float(dz_cmd),
